...
export default async function MetricDrillDownPage({
  ...
  const [series, drillDown] = await Promise.all([
    getMetricSnapshots(metric),
    getDrillDownRecords(metric),
  ]);
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-3 rounded-3xl border border-white/70 bg-white/60 p-5 shadow-sm backdrop-blur sm:p-6">
        ...
        <p className="text-sm text-muted-foreground">
          {drillDown.weekStart
            ? `${sortedRecords.length} GitHub record${sortedRecords.length === 1 ? "" : "s"} for the week of ${formatWeekLabel(drillDown.weekStart)} (the most recently ingested week) — nothing here is self-reported.`
            : "No ingested weeks yet for this squad."}
        </p>
        <div className="overflow-hidden rounded-2xl border border-white/70 bg-white/70 shadow-sm">
